...
public class ChangVersionAndPush {
    public static void main(String[] args) throws Exception {
        ...
        try (PrintWriter successWrite = new PrintWriter(FileUtils.openOutputStream(successLog, true))) {
            ...
            for (TisRepo repo : Main.tisRelsRepo) {
                if (passedRepos.contains(repo.repository) || repo.localDir == null || !repo.localDir.exist()) {
    ...
    public static void commitAndPushRepo(TisRepo repo) throws IOException, GitAPIException {
        ...
        // 2. 检查是否有未暂存的变更
        boolean hasUncommittedChanges = !status.getUntracked().isEmpty() ||    // 未跟踪文件
        ...
        if (hasUncommittedChanges) {
            ...
            git.commit().setMessage("set new version：v" + newVer).call();
            git.push()
    ...
    public static void updateAnsibleVersion(TisRepo repo) throws IOException {
        ...
        try (PrintWriter writer = new PrintWriter(new OutputStreamWriter(FileUtils.openOutputStream(varsFileNew),
    ...
    public static void updateVersion(TisRepo repo, List<VersionElement> versionElementMatchs) throws IOException {
        ...
        //        final Pattern PATTERN_REVISION = Pattern.compile("<revision>(" + StringUtils.replace(oldVer, ".",
        //        "\\.") + ")</revision>");
        //        final Pattern PATTERN_VERSION = Pattern.compile("<version>(" + StringUtils.replace(oldVer, ".", "\\
        //        .") + ")</version>");
        ...
        Matcher matcher = null;
        try (PrintWriter writer = new PrintWriter(new OutputStreamWriter(FileUtils.openOutputStream(pomFileNew),
                StandardCharsets.UTF_8))) {
    ...
    private static class VersionElement {
        ...
        public Pair<String, Pattern> createPattern(String targetVer) {
            if (this.patternElement == null) {
                ...
                this.patternElement = Pattern.compile("<" + patternElement + ">(" + StringUtils.replace(targetVer,
                        ".", "\\.") + ")</" + patternElement + ">");
            }
